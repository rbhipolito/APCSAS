...
if(xsrf_guard())
{
    ...
    if($_POST['btnSubmit'])
    {
        ...
        $mysqli->real_query("SELECT `username`, `skin_id`, `first_name`, `middle_name`, `last_name`, person.`person_id` FROM `user`, `person` WHERE `username`='$clean_username' AND `password`='$clean_password' AND `user`.`person_id` = `person`.`person_id`");
        if($result = $mysqli->use_result())
        {
            if($data = $result->fetch_assoc())
            {
                ...
                $dbh->set_table('person left join employee on person.person_id = employee.person_id
                    left join user on person.person_id = user.person_id');
                $dbh->set_where('person.person_id ="'.$_SESSION['person_id'].'" ');
                $dbh->exec_fetch('single');
                $dept = $dbh->dump['dept_id'];
...
// debug($_SESSION['dept_id']);
                ...
                $dbh = cobalt_load_class('person');
                $dbh->set_fields('student_no');
                $dbh->set_table('person left join student on person.person_id = student.person_id
                    left join user on person.person_id = user.person_id');
                $dbh->set_where('person.person_id ="'.$_SESSION['person_id'].'" ');
                $dbh->exec_fetch('single');
                $student_no = $dbh->dump['student_no'];
